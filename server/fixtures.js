 ...
 * Populating collections on the server. Health conditions will always be there, and not be edited by users
 * Links for descriptions found in readme
 */

if (HealthConditions.find().count() === 0)
{
    ...
    HealthConditions.insert({
        name: "Pregnancy Loss/Birth Defects",
        Description: "Some problems that occur in babies are due to genetic disorders. These disorders result from " +
        "abnormalities in one or more genes or in chromosomes (see Genes and Chromosomes and see Overview of Chromosomal Disorders). " +
        "Some abnormalities are hereditary. That is, they are passed down from generation to generation. Others—said to occur " +
        "spontaneously—result when genetic material in the parents’ sperm or egg cells or in the cells of the developing embryo is " +
        "damaged by chance or by drugs, chemicals, or other damaging substances (such as x-rays)." });
}
