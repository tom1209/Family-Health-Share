...
Meteor.methods({
    ...
    familyJoin : function(familyAttributes) {
        ...
        var familyIdToQuery = Families.findOne({familyID: familyAttributes.familyID});
        if (familyIdToQuery)
        {
            //Find the family name associated with that familyId
            var familyName = Families.findOne({familyID: familyAttributes.familyID});
            console.log(familyName.familyName);
        }
        else
        {
            return "There is no family with that ID";
        }
