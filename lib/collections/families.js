...
Meteor.methods({
    familyInsert: function(familyAttributes) {
        ...
        if (errors.familyID)
            throw new Meteor.Error('invalid-familyID', "Something went wrong when trying to create a new family");
        if (errors.familyName)
            throw new Meteor.Error('invalid-familyName', "Something went wrong when trying to create a new family");
        if (errors.familyPassword)
            throw new Meteor.Error('invalid-familyPassword', "Something went wrong when trying to create a new family");

        if (errors.passwordMismatch)
            throw new Meteor.Error('invalid-passwordMismatch', "Something went wrong when trying to create a new family");
